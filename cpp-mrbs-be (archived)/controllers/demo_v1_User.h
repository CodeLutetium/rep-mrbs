...
class User : public drogon::HttpController<User> {
  public:
    ...
    // /demo/v1/User/{arg2}/{arg1} METHOD_ADD(User::your_method_name,
    // "/{1}/{2}/list", Get); // path is /demo/v1/User/{arg1}/{arg2}/list
    // ADD_METHOD_TO(User::your_method_name, "/absolute/path/{1}/{2}/list",
    // Get);
    // // path is /absolute/path/{arg1}/{arg2}/list
    METHOD_ADD(User::login, "/token?userId={1}&password={2}", Post);
    METHOD_ADD(User::getInfo, "/{1}/info?token={2}", Get);
    METHOD_ADD(User::getRooms, "/rooms", Get);
    METHOD_LIST_END
    ...
    void getRooms(const HttpRequestPtr &req,
                  std::function<void(const HttpResponsePtr &)> &&callback);
};
} // namespace v1
} // namespace demo
